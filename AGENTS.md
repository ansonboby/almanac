...
- **Main-thread rule — know which calls move OFF main vs stay ON it (this is the inverse of the above, and copying the location fix to camera code reintroduces the same crash):** Play Services `Task`s (`FusedLocationProviderClient`, etc.) must move **OFF** the main thread (coroutine `.await()` inside `withContext(Dispatchers.IO)`). **CameraX must stay ON the main thread.** `ProcessCameraProvider.getInstance(ctx).addListener({ provider.bindToLifecycle(...) }, executor)` and `ImageCapture.takePicture(output, executor, callback)` run the binding/capture on whatever `executor` you pass — so that executor must be `ContextCompat.getMainExecutor(context)` (or `Dispatchers.Main.asExecutor()`), **never** `Executors.newSingleThreadExecutor()`. Passing a background executor throws exactly "Not in application's main thread" with a black preview box first. (Found in Phase 3: tapping Photo opened the camera, showed a black preview, then crashed — root cause was a background `Executor` on the camera binding, fixed by switching to the main executor. Verified on emulator: live preview renders, photo captures, saves, and shows on Today.)
